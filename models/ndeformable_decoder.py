...
from models.ndeformable_encoder import ScaledParameter
...
class DeformableDecoder(nn.Module):
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            query_pos = F.normalize(query_pos, p=2, dim=-1)
...
class DeformableDecoderLayer(nn.Module):
    def __init__(self,
        ...
        # Cross Attention
        self.cross_attn = nGPT_MSDeformAttn(
            d_model=d_model,
            n_levels=n_levels,
            n_heads=n_heads,
            n_points=n_points,
        ...
        self.alphaS = ScaledParameter(param_init=0.05, param_scale=1/torch.sqrt(torch.tensor(d_model, dtype=torch.float32)))
    ...
    def forward_self_attn(self, tgt, query_pos, query_mask):
        ...
        self.self_attn.in_proj_weight.data = F.normalize(self.self_attn.in_proj_weight.data, dim=-1)
        self.self_attn.out_proj.weight.data = F.normalize(self.self_attn.out_proj.weight.data, dim=-1)
        tgt = F.normalize(tgt + self.dropout2(self.alphaS()*(tgt2-tgt)), p=2, dim=-1)
        return tgt
    ...
    def forward_ffn(self, tgt):
        tgt1 = self.dropout3(self.activation(self.linear1(tgt*self.su())))
        tgt2 = self.linear2(tgt1*self.sv()*torch.sqrt(torch.tensor(tgt1.size(-1), dtype=torch.float32)))
        ...
        tgt = F.normalize(tgt + self.dropout4(self.alphaM()*(tgt2-tgt)), p=2, dim=-1)
        # tgt = self.norm3(tgt)
        return tgt
    ...
    def forward(self, tgt, query_pos, reference_points,
        ...
        tgt = F.normalize(tgt + self.dropout1(self.alphaA()*(tgt2-tgt)), p=2, dim=-1)
        # tgt = self.norm1(tgt)
        # FFN
