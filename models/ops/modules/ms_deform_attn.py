...
class nGPT_MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, sigmoid_attn=False, visualize=False):
        """
        Multi-Scale Deformable Attention Module
        :param d_model      hidden dimension
    ...
    def forward(self, query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask=None):
        ...
        value = self.value_proj(input_flatten)
        self.value_proj.weight.data = F.normalize(self.value_proj.weight.data, p=2, dim=-1)
        ...
        output = MSDeformAttnFunction.apply(
            value, input_spatial_shapes, input_level_start_index, sampling_locations, attention_weights, self.im2col_step)
        output = self.output_proj(output)
        self.output_proj.weight.data = F.normalize(self.output_proj.weight.data, p=2, dim=-1)
        output = F.normalize(output, p=2, dim=-1)
        self.attention_weights.weight.data = F.normalize(self.attention_weights.weight.data, p=2, dim=-1)
        return output
