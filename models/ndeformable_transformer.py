...
from .ndeformable_encoder import DeformableEncoderLayer, DeformableEncoder, ScaledParameter
from .ndeformable_decoder import DeformableDecoderLayer, DeformableDecoder
...
class DeformableTransformer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        self.logits_rescaler = nn.ModuleList([ScaledParameter(1.0, 1.0/torch.sqrt(torch.tensor(d_model, dtype=torch.float32))) for _ in range(n_dec_layers)])
    ...
    def forward(self, srcs: List[torch.Tensor], masks: List[torch.Tensor],
        ...
        for level in range(output.shape[0]):
            tmp.append(output[level]*self.logits_rescaler[level]())
